...
#ifndef _MOEX_SPECTRA_SIMBA_SEQUENCERESET_H_
#define _MOEX_SPECTRA_SIMBA_SEQUENCERESET_H_
...
public:
    ...
    SequenceReset(char *buffer, const std::uint64_t bufferLength) :
        SequenceReset(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SequenceReset(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SequenceReset(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return *this = SequenceReset(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    SequenceReset &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    SequenceReset &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = SequenceReset(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }
    ...
    {
        SequenceReset skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
...
#pragma clang diagnostic pop
    ...
    SBE_NODISCARD std::uint32_t newSeqNo() const SBE_NOEXCEPT
    {
        std::uint32_t val;
    ...
    SequenceReset &newSeqNo(const std::uint32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits>& operator << (
    std::basic_ostream<CharT, Traits>& builder, SequenceReset _writer)
{
    SequenceReset writer(_writer.m_buffer, _writer.m_offset,
        _writer.m_bufferLength, _writer.sbeBlockLength(), _writer.m_actingVersion);
    builder << '{';
    builder << R"("Name": "SequenceReset", )";
    builder << R"("sbeTemplateId": )";
    builder << writer.sbeTemplateId();
    builder << ", ";

    builder << R"("NewSeqNo": )";
    builder << +writer.newSeqNo();
